...
public:
    ...
    ConvertPrecision(ngraph::element::Type_t from, ngraph::element::Type_t to, 
                        std::map<ngraph::NodeTypeInfo, std::function<bool(std::shared_ptr<Node>&, element::Type, size_t idx)>> additional_type_to_fuse_map = {})
        : FunctionPass(),
        m_from(from),
        m_to(to),
...
private:
    ...
    std::map<ngraph::NodeTypeInfo, std::function<bool(std::shared_ptr<Node>&, element::Type, size_t idx)>> m_additional_type_to_fuse_map;
};
