...
void FrontEnd::parseSplit(const Model& model, const NodePtr& node, const DataVector& inputs, const DataVector& outputs) const {
    ...
    const auto axisNodeСonst = ngraph::as_type_ptr<ngraph::op::Constant>(axisNode);
    VPU_THROW_UNLESS(axisNodeСonst != nullptr, "Can't parse node with name %s and type %s. Can't get axis node as const", node->get_friendly_name(), node->get_type_name());
    const auto axisIE = axisNodeСonst->get_vector<int64_t>()[0];
